...
const pubPrivRegex = /(pub|priv)\s+(contract|resource|struct|event|enum|case|entitlement|view|fun|var|let|init)/gimu;
...
function preProcessCadence1(data){
  ...
  // data = replaceSubstitutedEscaped(data, pubPrivRegex, (match)=>`access(${match[1]==='pub'?'all':'self'}) ${match[2]}`)
